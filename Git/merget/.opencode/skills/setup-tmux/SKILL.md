...
1. Determine the working directory:
   - If you are currently working in a worktree (e.g., `ZS--my-feature/`, `MAIN/`), use that worktree's absolute path.
   - If no worktree context has been established, use the project root (`/Users/zachiahsawyer/Git/merget`).

2. Run the script, passing the directory as the only argument:

   ```bash
   /Users/zachiahsawyer/Git/merget/.opencode/skills/setup-tmux/setup-tmux.sh '{dir}'
   ```
